...
int ndpi_ipv4_fmt(void *dst,const int dstlen,const u_int32_t src)
{
    ndpi_ip4_address_t source;
    source.address = src;
    memset(dst,0,dstlen);
    snprintf((char *)dst,dstlen,"%u.%u.%u.%u",source.addr_u8[0],source.addr_u8[1],source.addr_u8[2],source.addr_u8[3]);
    return 0;
}

int ndpi_ipv6_fmt(void *dst,const int dstlen,const u_int32_t src[])
{
    int i;
    ...
    {
        msw = src[i] >> 16;
        lsw = src[i] & 0x0000ffff;
        addr_dst_str += sprintf(addr_dst_str,"%X:%X:",msw,lsw);
    }
    *(addr_dst_str - 1) = '\0';
...
static int ndpi_find_non_eng_bigrams(struct ndpi_detection_module_struct *ndpi_struct, char *str)
{
    char s[3];
...
int ndpi_has_human_readeable_string(struct ndpi_detection_module_struct *ndpi_struct,char *buffer, u_int buffer_size,
                                    u_int8_t min_string_match_len,char *outbuf, u_int outbuf_len) 
{
                   ...
                   ndpi_serializer *serializer)
{
    ...
    if(ip_version == 4) {
        ndpi_ipv4_fmt(src_name,sizeof(src_name),src_v4);
        ndpi_ipv4_fmt(dst_name,sizeof(dst_name),dst_v4);
    } else {
        ndpi_ipv6_fmt(src_name,sizeof(src_name),src_v6->u6_addr.u6_addr32);
        ndpi_ipv6_fmt(dst_name,sizeof(dst_name),dst_v6->u6_addr.u6_addr32);
    ...
    case NDPI_PROTOCOL_DNS:
        ...
        ndpi_ipv4_fmt(buf,sizeof(buf),flow->protos.dns.rsp_addr.ipv4);
